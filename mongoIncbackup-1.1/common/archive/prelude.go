...
// DirLike represents the group of methods done on directories and files in backup directories,
...
func (prelude *Prelude) Read(in io.Reader) error {
	...
	if readMagicNumber != MagicNumber {
		return fmt.Errorf("stream or file does not appear to be a mongobackup archive")
	}
...
//
//  "backup/oplog.bson"     => &PreludeExplorer{ database: "", collection: "oplog.bson" }
//  "backup/test/"          => &PreludeExplorer{ database: "test", collection: "foo.bson" }
//  "backup/test/foo.bson"  => &PreludeExplorer{ database: "test", collection: "" }
//  "backup/test/foo.json"  => &PreludeExplorer{ database: "test", collection: "foo", isMetadata: true }
//
