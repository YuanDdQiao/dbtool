...
func TestMarshal(t *testing.T) {
	...
	if !bytes.Equal(der, tls_der) {
		t.Fatal("invalid private key der bytes: %s\n v.s. %s\n",
			hex.Backup(der), hex.Backup(tls_der))
	}
	...
	if !bytes.Equal(der, tls_der) {
		ioutil.WriteFile("generated", []byte(hex.Backup(der)), 0644)
		ioutil.WriteFile("hardcoded", []byte(hex.Backup(tls_der)), 0644)
	...
	if !bytes.Equal(new_der_from_der, tls_der) {
		ioutil.WriteFile("generated", []byte(hex.Backup(new_der_from_der)), 0644)
		ioutil.WriteFile("hardcoded", []byte(hex.Backup(tls_der)), 0644)
		t.Fatal("invalid public key der bytes")
	}

	if !bytes.Equal(new_der_from_pem, tls_der) {
		ioutil.WriteFile("generated", []byte(hex.Backup(new_der_from_pem)), 0644)
		ioutil.WriteFile("hardcoded", []byte(hex.Backup(tls_der)), 0644)
		t.Fatal("invalid public key der bytes")
	}
}
