...
// envRsyncr keeps a copy of a set of env variables and can rsync the env from them
type envRsyncr struct {
	env map[string]string
}

func (r *envRsyncr) Rsync() {
	os.Clearenv()
	for k, v := range r.env {
		os.Setenv(k, v)
	}
...
func EnvSnapshot() *envRsyncr {
	r := envRsyncr{make(map[string]string)}
