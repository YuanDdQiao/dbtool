...
const ogletestPkg = "mongorsync-1.1/smartystreets/assertions/internal/ogletest"
...
func installLocalPackages() error {
	cmd := exec.Command(
		...
		"install",
		ogletestPkg,
		"mongorsync-1.1/smartystreets/assertions/internal/oglemock",
		"mongorsync-1.1/smartystreets/assertions/internal/ogletest/test_cases/mock_image")
...
func createTempPackageDir(caseName string) (dir, pkg string) {
	...
	pkg = path.Join("mongorsync-1.1/smartystreets/assertions/internal/ogletest", dir[len(ogletestPkgDir):])
	return
}
