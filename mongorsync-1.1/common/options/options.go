...
import (
	...
	"mongorsync-1.1/jessevdk/go-flags"
...
type FNamespace struct {
	...
	FDB         string `short:"D" long:"fdb" value-name:"<database-name>" description:"database to use"`
	FCollection string `short:"C" long:"fcollection" value-name:"<collection-name>" description:"collection to use"`
}
...
type Connection struct {
	...
	FHost string `short:"H" long:"fhost" value-name:"<hostname>" description:"mongodb host to connect to (setname/host1,host2 for replica sets)"`
	FPort string `long:"fport" value-name:"<port>" description:"server port (can also use --host hostname:port)"`
}
...
type SSL struct {
	...
	FUseSSL              bool   `long:"ssl" description:"connect to a mongod or mongos that has ssl enabled"`
	FSSLCAFile           string `long:"sslCAFile" value-name:"<filename>" description:"the .pem file containing the root certificate chain from the certificate authority"`
	FSSLPEMKeyFile       string `long:"sslPEMKeyFile" value-name:"<filename>" description:"the .pem file containing the certificate and key"`
	FSSLPEMKeyPassword   string `long:"sslPEMKeyPassword" value-name:"<password>" description:"the password to decrypt the sslPEMKeyFile, if necessary"`
	FSSLCRLFile          string `long:"sslCRLFile" value-name:"<filename>" description:"the .pem file containing the certificate revocation list"`
	FSSLAllowInvalidCert bool   `long:"sslAllowInvalidCertificates" description:"bypass the validation for server certificates"`
	FSSLAllowInvalidHost bool   `long:"sslAllowInvalidHostnames" description:"bypass the validation for server name"`
	FSSLFipsMode         bool   `long:"sslFIPSMode" description:"use FIPS mode of the installed openssl library"`
}
...
type Auth struct {
	...
	FUsername  string `short:"U" value-name:"<username>" long:"fusername" description:"username for authentication"`
	FPassword  string `short:"P" value-name:"<password>" long:"fpassword" description:"password for authentication"`
	FSource    string `long:"fauthenticationDatabase" value-name:"<database-name>" description:"database that holds the user's credentials"`
	FMechanism string `long:"fauthenticationMechanism" value-name:"<mechanism>" description:"authentication mechanism to use"`
}
...
type Kerberos struct {
	...
	//from source kerberos
	FService     string `long:"fgssapiServiceName" value-name:"<service-name>" description:"service name to use when authenticating using GSSAPI/Kerberos ('mongodb' by default)"`
	FServiceHost string `long:"fgssapiHostName" value-name:"<host-name>" description:"hostname to use when authenticating using GSSAPI/Kerberos (remote server's address by default)"`
}
...
func New(appName, usageStr string, enabled EnabledOptions) *ToolOptions {
	...
	/*	// from source register options
		if enabled.FConnection {
			if _, err := opts.parser.AddGroup("fconnection options", "", opts.Connection); err != nil {
				panic(fmt.Errorf("couldn't register connection options: %v", err))
			}
	...
	*/
	...
	/*	if enabled.FAuth {
		if _, err := opts.parser.AddGroup("fauthentication options", "", opts.Auth); err != nil {
			panic(fmt.Errorf("couldn't register auth options"))
		}
	}*/
	if enabled.FNamespace {
		if _, err := opts.parser.AddGroup("namespace options", "", opts.FNamespace); err != nil {
...
func (auth *Auth) FRequiresExternalDB() bool {
	return auth.FMechanism == "GSSAPI" || auth.FMechanism == "PLAIN" || auth.FMechanism == "MONGODB-X509"
}
...
func (auth *Auth) FShouldAskForPassword() bool {
	return auth.FUsername != "" && auth.FPassword == "" &&
		!(auth.FMechanism == "MONGODB-X509" || auth.FMechanism == "GSSAPI")
}
...
func (o *ToolOptions) GetAuthenticationDatabase() string {
	if o.Auth.Source != "" {
		...
		return "$external"
	} else if o.FNamespace != nil && o.FNamespace.DB != "" {
		return o.FNamespace.DB
	}
	return ""
}
func (o *ToolOptions) FGetAuthenticationDatabase() string {
	if o.Auth.FSource != "" {
		return o.Auth.FSource
	} else if o.Auth.FRequiresExternalDB() {
		return "$external"
	} else if o.FNamespace != nil && o.FNamespace.DB != "" {
		return o.FNamespace.DB
	}
