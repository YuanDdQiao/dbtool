// Package intents provides utilities for performing dump/rsync operations.
package intents
...
// mongorsync first scans the directory to generate a list
// of all files to rsync and what they map to. TODO comments
type Intent struct {
	// Namespace info
	DB string
	C  string
	...
	/*	BSONFile     file
		...
		MetadataFile file
	*/
	sessionprod *db.SessionProvider
...
type Manager struct {
	...
	// legacy mongorsync works in the order that paths are discovered,
...
// This can be used to check for possible unwanted conflicts before rsyncing to a sharded system.
...
// Finish tells the prioritizer that mongorsync is done rsyncing
...
// stored with the other intents, because it is dumped and rsyncd in
...
// Users returns the intent of the users collection to rsync, a special case
func (manager *Manager) Users() *Intent {
	return manager.usersIntent
}

// Roles returns the intent of the user roles collection to rsync, a special case
func (manager *Manager) Roles() *Intent {
	return manager.rolesIntent
}

// AuthVersion returns the intent of the version collection to rsync, a special case
