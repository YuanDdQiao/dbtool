...
// process of being rsyncd through the "Finish" hook.
//
// Oplog entries and auth entries are not handled by the prioritizer,
// as these are special cases handled by the regular mongorsync code.
...
// file system, keeping with legacy mongorsync behavior.
...
// database's intents being rsyncd as possible and attempts to rsync the
...
// it is better to have two rsync jobs where
...
// run first. Admittedly, .bson file size is not a direct predictor of rsync
...
// rsyncs for the returned intent's DB. Get is not thread safe, and depends
...
// Finish decreases the number of active rsync jobs for the given intent's
...
// Returns the largest collection of the databases with the least active rsyncs.
