...
import (
	...
	"mongorsync-1.1/common/password"
...
func FNewSessionProvider(opts options.ToolOptions) (*SessionProvider, error) {
	// create the provider
	provider := &SessionProvider{
		readPreference:           mgo.Primary,
		bypassDocumentValidation: false,
	...
	// finalize auth options, filling in missing passwords
	if opts.Auth.FShouldAskForPassword() {
		opts.Auth.FPassword = password.Prompt()
	}
	...
	// configure the connector
	err := provider.connector.FConfigure(opts)
	if err != nil {
		return nil, fmt.Errorf("error configuring the connector: %v", err)
	}
	return provider, nil
