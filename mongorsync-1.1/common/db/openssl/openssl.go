...
import (
	...
	"mongorsync-1.1/spacemonkeygo/openssl"
)
...
func (self *SSLDBConnector) FConfigure(opts options.ToolOptions) error {
	// create the addresses to be used to connect
	...
	var flags openssl.DialFlags
	flags = 0
	if opts.FSSLAllowInvalidCert || opts.FSSLAllowInvalidHost || opts.FSSLCAFile == "" {
		flags = openssl.InsecureSkipHostVerification
	}
	// create the dialer func that will be used to connect
	dialer := func(addr *mgo.ServerAddr) (net.Conn, error) {
...
func fsetupCtx(opts options.ToolOptions) (*openssl.Ctx, error) {
	var ctx *openssl.Ctx
	var err error
	...
	// add the PEM key file with the cert and private key, if specified
	if opts.FSSLPEMKeyFile != "" {
		if err = ctx.UseCertificateChainFile(opts.FSSLPEMKeyFile); err != nil {
			return nil, fmt.Errorf("UseCertificateChainFile: %v", err)
		}
		if opts.FSSLPEMKeyPassword != "" {
			if err = ctx.UsePrivateKeyFileWithPassword(
				opts.FSSLPEMKeyFile, openssl.FiletypePEM, opts.FSSLPEMKeyPassword); err != nil {
				return nil, fmt.Errorf("UsePrivateKeyFile: %v", err)
			}
		} else {
			if err = ctx.UsePrivateKeyFile(opts.FSSLPEMKeyFile, openssl.FiletypePEM); err != nil {
				return nil, fmt.Errorf("UsePrivateKeyFile: %v", err)
			}
		}
		// Verify that the certificate and the key go together.
	...
	if opts.FSSLCAFile != "" {
		calist, err := openssl.LoadClientCAFile(opts.FSSLCAFile)
		if err != nil {
			return nil, fmt.Errorf("LoadClientCAFile: %v", err)
		}
		ctx.SetClientCAList(calist)

		if err = ctx.LoadVerifyLocations(opts.FSSLCAFile, ""); err != nil {
			return nil, fmt.Errorf("LoadVerifyLocations: %v", err)
		}

		var verifyOption openssl.VerifyOptions
		if opts.FSSLAllowInvalidCert {
			verifyOption = openssl.VerifyNone
		} else {
			verifyOption = openssl.VerifyPeer
		}
	...
	if opts.FSSLCRLFile != "" {
		...
		lookup.LoadCRLFile(opts.FSSLCRLFile)
	}
